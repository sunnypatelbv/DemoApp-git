...
//
//   let addToCartModel = try? JSONDecoder().decode(AddToCartModel.self, from: jsonData)
...
struct AddToCartModel: Codable {
    ...
    let cCashback: CCashback?
    let cPointsInfo: CPointsInfo?
    let cCashbackPoints: CCashbackPoints?
    let cSavings: CCashback?
    let cAmount: CAmount?
    let cCouponDiscount: Int?
    let cCouponsAutoApplied, cCouponsApplied: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case cCashback = "c_cashback"
        case cPointsInfo = "c_pointsInfo"
        case cCashbackPoints = "c_cashbackPoints"
        case cSavings = "c_savings"
        case cAmount = "c_amount"
        case cCouponDiscount = "c_couponDiscount"
        case cCouponsAutoApplied = "c_couponsAutoApplied"
        case cCouponsApplied = "c_couponsApplied"
        case baskets
    }
}
// MARK: - Basket
struct Basket: Codable {
    ...
    let agentBasket: Bool?
    let basketID, channelType, creationDate, currency: String?
    let customerInfo: CustomerInfo?
    let lastModified: String?
    let merchandizeTotalTax: Double?
    let notes: Notes?
    let orderTotal: JSONNull?
    let productItems: [ProductItem]?
    let productSubTotal, productTotal: Int?
    let shipments: [Shipment]?
    let shippingItems: [ShippingItem]?
    ...
    let cCashBackTotalAmount: Double?
...
// MARK: - CAmount
struct CAmount: Codable {
    let subTotalRaw, subTotal, discountRaw, discount: String?
    let totalDiscountRaw, totalDiscount, discountText: String?
    let totalRaw: Int?
    let total, discountPercentageRaw, totalDiscountPercentageRaw, shippingRaw: String?
    let shipping, taxRaw, tax, paymentRaw: String?
    let payment: String?
}

// MARK: - CCashback
struct CCashback: Codable {
    let value: Int?
    let formatted, message: String?
    let cashBackTooltipMessage: JSONNull?
}

// MARK: - CCashbackPoints
struct CCashbackPoints: Codable {
    let value: Int?
    let collectionStatus, isCashbackPointCollected, collectablePointStatus: JSONNull?
    let cashBackPointMessage, cashBackPointValueMessage: CashBackPointMessage?
    let availableCashBackPointAmount: Int?
    let availableCashBackPointMessage: JSONNull?
    let isLoyaltyPointsApplied, isLoyaltyToggleVisible: Bool?
}

// MARK: - CashBackPointMessage
struct CashBackPointMessage: Codable {
    let label: String?
}

// MARK: - CPointsInfo
struct CPointsInfo: Codable {
    let isToggle: Bool?
    let pointReward, pointRewardText: String?
}

// MARK: - CustomerInfo
struct CustomerInfo: Codable {
    let type, customerID, customerNo, email: String?
...
//    let type, flashType, message, path: String?
//
//    enum CodingKeys: String, CodingKey {
//        case type = "_type"
//        case flashType = "type"
